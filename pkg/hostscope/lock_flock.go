...
// lockExclusive holds f's advisory lock until it is granted or ctx ends.
//
...
//
// ON THE ctx PATH THIS FUNCTION OWNS f. The flock syscall has no deadline, so
// a caller that gives up leaves a lock request outstanding that will be granted
// later whether anyone wants it or not — and a grant nobody unlocks is the
// machine wedged. The cleanup goroutine below is the only party that can know
// when that grant lands, so it takes the descriptor with it: it unlocks
// whatever it was given and closes. The caller must not touch f after an error.
func lockExclusive(ctx context.Context, f *os.File) (contended bool, err error) {
	fd := int(f.Fd())

	// Ask without blocking first, so an uncontended acquire is KNOWN to be
	// uncontended rather than measured as a very short wait. The difference is
	// not cosmetic: the caller files a non-zero wait to the ledger, and a
	// microsecond charged as contention on every run would put a write on the
	// orchestrator for each one and report queueing on a machine where nothing
	// queued. A clock cannot tell those apart; this syscall can.
	...
	case err := <-granted:
		if err != nil {
			f.Close()
			return true, fmt.Errorf("hostscope: cannot take the host-scope exclusion on %s: %w", f.Name(), err)
		}
		return true, nil
	case <-ctx.Done():
		go func() {
		...
		return true, fmt.Errorf("hostscope: gave up waiting for the host-scope exclusion on %s: %w", f.Name(), ctx.Err())
	}
}
