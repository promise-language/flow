...
func TestWriteContract_BranchViolation(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		...
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}, Writes: flow.WriteContract{}}) // zero = writes nothing
	}, &stubAgent{name: "stub"})
...
func TestWriteContract_CommitViolation(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("plan", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			...
			return ctx.Finalize(flow.DispositionResolved, "done").Markdown("done"), nil
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}, Writes: flow.WriteContract{}})
	}, &stubAgent{name: "stub"})
...
func TestWriteContract_AllowedCommit(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("impl", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			...
			return ctx.Finalize(flow.DispositionResolved, "done").Markdown("done"), nil
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}, Writes: flow.WriteContract{MayCommit: true}})
	}, &stubAgent{name: "stub"})
...
func TestWriteContract_TransientSkipsCheck(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		...
		}, flow.StepConfig{Entry: true, Writes: flow.WriteContract{}})
	}, &stubAgent{name: "stub"})
...
func TestWriteContract_AllowedBranch(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("branch", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			...
			return ctx.Finalize(flow.DispositionResolved, "done").Markdown("done"), nil
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}, Writes: flow.WriteContract{MayBranch: true}})
	}, &stubAgent{name: "stub"})
...
func TestWriteContract_AllowedDirtyTree(t *testing.T) {
	app, be, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("edit", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			...
			return ctx.Finalize(flow.DispositionResolved, "done").Markdown("done"), nil
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}, Writes: flow.WriteContract{MayEditTree: true}})
	}, &stubAgent{name: "stub"})
...
func TestWriteContract_PartialContract_CommitAllowedBranchNot(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("impl", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			...
			return ctx.Finalize(flow.DispositionResolved, "done").Markdown("done"), nil
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}, Writes: flow.WriteContract{MayCommit: true}}) // MayBranch defaults false
	}, &stubAgent{name: "stub"})
...
// about that.
func resultEntry(step, next flow.StepId, exec int, body flow.ArtifactBody) flow.JournalEntry {
	return flow.JournalEntry{
		Step:      step,
		Execution: exec,
		Result:    body,
		// The successor is named by the caller, because position is derived from
		// it: a seeded journal electing a step the flow does not register is an
		// item the advance refuses rather than one standing anywhere.
...
func appendResult(t *testing.T, o flow.Orchestrator, ref flow.ItemRef, step, next flow.StepId, exec int, body flow.ArtifactBody) {
	t.Helper()
	if err := o.AppendEntry(context.Background(), ref, resultEntry(step, next, exec, body)); err != nil {
		t.Fatalf("AppendEntry(%s exec %d): %v", step, exec, err)
	}
}

// appendMarkdown is appendResult for the common markdown case.
func appendMarkdown(t *testing.T, o flow.Orchestrator, ref flow.ItemRef, step, next flow.StepId, text string) {
	t.Helper()
	appendResult(t, o, ref, step, next, 1, flow.ArtifactBody{Type: flow.ArtifactMarkdown, Markdown: text})
}
...
// A wait has no handler, so an advance that reaches one dispatches nothing and
// spends nothing: the item sits there, nobody's move.
//
// It sits there after the signal is observed, too. Position reads the journal
// and the journal alone, and nothing appends a wait's own entry — so the route
// does not move past it. That is #238's remit (a wait as a routed node whose
// entry is appended on the observation); what this pins is that reaching one
// costs nothing either way.
func TestRunOne_AwaitSignalSkipsHandlerless(t *testing.T) {
	app, be, claim := testApp(t, func(f *flow.Flow) {
		f.AwaitSignal("await merge", "pr-open", flow.StepConfig{Entry: true})
	}, &stubAgent{name: "stub"})
	...
	if res.Status != "skipped" {
		t.Errorf("after signal = %+v, want skipped — nothing appends the wait's entry (#238)", res)
	}
	state, _ := be.Load(context.Background(), claim.ItemRef)
	if len(state.Journal) != 0 {
		t.Errorf("journal = %+v, want empty — a wait dispatches nothing, so nothing completes", state.Journal)
	}
	if got := state.Ledger.Row("pr-open").Dispatches; got != 0 {
		t.Errorf("Dispatches = %d on the wait, want 0 — reaching a wait spends nothing", got)
	}
}
...
func TestSelectFlow_RequireSignalGate(t *testing.T) {
	...
	f.AddStep("merge", "commit", func(flow.StepCtx) (flow.StepResult, error) { return flow.StepResult{}, nil },
	...
	if picked, _, err := SelectFlow(app, state); err != nil || picked != nil {
		t.Errorf("SelectFlow = (%v, %v) without pr-open; want no eligible flow and no error", picked, err)
	}

	state.Signals["pr-open"] = flow.SignalState{Set: true}
	picked, next, err := SelectFlow(app, state)
	if err != nil {
		t.Fatalf("SelectFlow: %v", err)
	}
...
func TestRunOne_PlainPreflightErrorStillSkips(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("never runs", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) { return flow.StepResult{}, nil }, flow.StepConfig{Entry: true})
	}, &stubAgent{name: "stub"})
	app.Preflight = func(context.Context, *flow.Item) error {
...
// ---------------------------------------------------------------------------
// The advance reads the route, and nothing else.
// ---------------------------------------------------------------------------

// An empty journal pends the DECLARED entry step — not the first registration,
// which is what the checklist used to answer with.
func TestRunOne_AnEmptyJournalDispatchesTheEntryStep(t *testing.T) {
	app, _, claim := testApp(t, func(f *flow.Flow) {
	...
	if res.Step != "plan" {
		t.Errorf("dispatched %q, want the declared entry step", res.Step)
	}
}

// The route names the step, and a result already recorded is not a reason to
// skip it: "reaching a step a second time is not an anomaly but a route". This
// is the rework handback, which the checklist could not express at all — it
// returned the first UNRESOLVED item, so an edge back to a completed step could
// never fire.
func TestRunOne_ARouteToAnAlreadyCompletedStepDispatchesItAgain(t *testing.T) {
	runs := 0
	app, be, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("write plan", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			runs++
	...
	// One completed execution of the step, electing itself — the shape a
	// handback has: the successor's result is already on the item.
	appendMarkdown(t, be, claim.ItemRef, "plan", "plan", "the first plan")
	...
	if res.Status != "done" || res.Step != "plan" {
		t.Fatalf("res = %+v, want the plan step dispatched again", res)
	}
	if runs != 1 {
		t.Errorf("the handler ran %d time(s) this dispatch, want 1", runs)
	}
	state, _ := be.Load(context.Background(), claim.ItemRef)
	if got := state.Artifact("plan").Markdown; got != "the second plan" {
		t.Errorf("plan = %q, want the later execution's result to stand", got)
	}
}

// A finalizing entry ends the flow, and the disposition it carried is what the
// item is finalized as. There is no completion test beside the route: nothing
// counts results, and a `rejected` item must not be recorded as resolved.
func TestRunOne_AFinalizingEntryFinalizesWithItsOwnDisposition(t *testing.T) {
	for _, d := range []flow.Disposition{flow.DispositionResolved, flow.DispositionRejected} {
		t.Run(string(d), func(t *testing.T) {
			...
			if err := be.AppendEntry(context.Background(), claim.ItemRef, flow.JournalEntry{
				Step: "plan", Execution: 1, Route: flow.Route{Finalize: d}, By: "tester",
			}); err != nil {
			...
			if res.Status != "done" || !res.Finalized {
				t.Fatalf("res = %+v, want a finalized item", res)
			}
			if wrapped.disposition != d {
				t.Errorf("finalized as %q, want the disposition the election carried (%q)",
					wrapped.disposition, d)
			}
		})
	}
}

// A route naming no registered step is the flow's own defect, and it comes back
// as one. Answering it empty would read as "nothing left to do" and finalize an
// item whose route the flow cannot follow — the one answer that would finish an
// item the flow never ran.
func TestRunOne_ARouteToAnUnregisteredStepSurfacesTheRefusal(t *testing.T) {
	app, be, claim := testApp(t, func(f *flow.Flow) {
		f.AddStep("write plan", "plan", func(ctx flow.StepCtx) (flow.StepResult, error) {
			return ctx.Finalize(flow.DispositionResolved, "done").Markdown("the plan"), nil
		}, flow.StepConfig{Entry: true, MayFinalize: []flow.Disposition{flow.DispositionResolved}})
	...
	appendMarkdown(t, be, claim.ItemRef, "plan", "no-such-step", "the plan")

	res, err := RunOne(context.Background(), app, claim)
	if err == nil {
		t.Fatalf("RunOne = (%+v, nil), want the route refused", res)
	}
	if !strings.Contains(err.Error(), "no-such-step") {
		t.Errorf("err = %q, want it to name the successor that resolves to nothing", err)
	}
	if wrapped.finalizeCalls != 0 {
