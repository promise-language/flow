...
Not required. A project that has these should call them these, so that a reader moving between projects — or an agent working in one it has not seen — does not have to learn a private vocabulary.
