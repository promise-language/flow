...
**A correction round for a refused result runs inside the dispatch that was refused, once.** The step is re-run in the same dispatch with what was refused and why in hand (§ Drafts: the text to keep *is* the refused text), and nothing is journaled until the guard accepts what it produces. A result refused again after that round **parks for a person**: a second refusal of the same work is a loop, not a transient, and the park says so by classifying as one no re-dispatch clears. What was refused is kept with the step, unpublished, and a person's re-run buys the next round. A dispatch that could not keep what was refused parks *before* spending a round — a round without the record could not differ from the attempt before it — and the next dispatch starts over. **A spent round is never renewed by the park that reports it**: once the round is spent the park is the one a person clears, whether or not the last refusal could be kept — a park inviting a re-dispatch would hand the next one a fresh round, and since no refusal is charged a dispatch there is nothing else to stop it. A person's re-run carries a round because a person chose to buy it, which is what makes it a decision rather than a loop nobody is watching.
