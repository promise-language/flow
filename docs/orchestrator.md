...
**An orchestrator runs no agent process.** Where the prompts of its arenas run on a host it dispatches to, it supplies the connection between the requester's client `Agent` and the prompt host its runner embeds, and nothing else: spawning, containment, the deadline, the refusal of a second prompt and the classification of how a prompt ended are the SDK's, at both ends ([agent.md](agent.md) § Where a prompt runs). An orchestrator or runner that handled the process, or read the host's answers into failures of its own, would be a second route to the agent substrate ([resolution.md](resolution.md) § One seam per outside service).
...
| `Claim` | A held lease **on one item** | The credentialed handle returned by `Orchestrator.Claim`. **It is never a parameter.** It carries the `ItemRef`, the `AccountId` credited, and the orchestrator-internal `Token`, and it is returned — by `Claim` and by `LookupActiveClaim` — never passed back in. Every method addresses the item by `ItemRef`; holding the lease is a **precondition the orchestrator checks against the claim it currently has**, not a value a caller supplies. A supplied claim is a value, and values go stale: The `already-held` override lets another arena take an item over, leaving the first holding a struct that still looks valid. A write that carried its own proof would be writing under revoked authority. Serialized to `.flow/active.json` and recoverable via `LookupActiveClaim`. It leases an item and nothing else: the other two exclusions in this system — the host-scope exclusion that serializes a declared measurement, and the project-scope lock that serializes landing — are **not `Claim`s**, carry no token and authorise nothing, and are **the orchestrator's** for the reason the claim is, that only a party seeing more than one arena can serialize them. Both are defined in [gates-and-commands.md](gates-and-commands.md) § Two scopes and reached through this contract's worktree surface — host scope through `RunGate` and `Run`; project scope through the **landing act**, which is `Push` where the push *is* the integration and `Merge` where the orchestrator lands through a request ([resolution-standalone.md](resolution-standalone.md) § Where verify is required: a branch push is not trunk). **The exclusion spans the round and not that act**, so it is taken at the first act of the round — `PrepareMergeResult`, where the orchestrator has one — and given back at the landing act. It carries the `(HostId, ArenaId)` of the arena it binds — a handle to a lease that could not name what the lease binds would leave every holder unidentifiable wherever one account runs more than one arena. |
| `AccountId` | An account in the orchestrator's own namespace | For GitHub, a login. The **runner's** account is **never passed in; always derived** from the credentials the arena acts as: an arena that can commit, push and merge has exactly one account by construction, so the orchestrator reads it rather than being told, and there is no way for a caller's idea of the account to differ from the one the work is actually done as. Non-empty. It becomes a label (`flow:owner:<account>`) and an assignee, so it carries the same floor as `TagId`. It credits claims and stamps journal entries — the per-role accounts of record are derived from those entries ([resolution.md](resolution.md)) — and the item's **creator** is one too, reported on `Item`. It is not what the lease binds to: a claim is `item ↔ arena`, never `item ↔ user`. Neither `Claim` nor `LookupActiveClaim` takes one. |
| `AgentAccountId` | The account the **agent substrate** spends as | The stable identifier the substrate issues, in the substrate's own namespace. **Not `AccountId`** — that one is the orchestrator's login, which credits claims and backs roles; this one pays for the prompts, and [cli.md](cli.md) § The announcement names the run's standing keeps them separate axes: *paying for a run and being permitted to perform it are different questions.* A host may drive several of these under one of those. It is **read from a stated field in a stated place, never discovered** by scanning configuration for something that looks like an account, and **never synthesized** from a host name, a path or a token — a reader that cannot establish it says so, and the empty value is not an identity and must never be compared as one ([environment.md](environment.md) § The agent account). A display name may accompany it so a person can read a report; it is never what a comparison is made on, and never what crosses the wire. It scopes an `account-exhausted` park and nothing else: no method addresses anything with it. |
...
| `(HostId, ArenaId)` | An arena | The pair is the arena identity, and the only form unique beyond a single machine. **`ArenaId` alone is not an identity** — it is a component, unique only within its host, so it names an arena no more than a house number names a house. It is a worktree plus a stable identity, where work physically happens, and it must be **stable across restarts**: it outlives at least one item's full lifecycle, because the claim it holds does. An orchestrator enforcing one-to-one claims compares the pair. For the GitHub orchestrator both halves are derived rather than configured — the arena is the local checkout — and `.flow/active.json` scopes one direction of the binding only: one file per checkout answers "what does **this** arena hold?" and cannot answer "does another arena hold this item?", because the arena next door cannot see the file. **The item must therefore carry the arena too.** It carries it as a fingerprint, not as the pair — see [github-schema.md](github-schema.md) § Labels, `flow:arena:<fingerprint>` — since publishing a machine name and an absolute worktree path is what [disclosure.md](disclosure.md) exists to prevent, and a digest decides the equality the exclusion needs without disclosing either half. |
| `ArtifactId` | A step-produced artifact — and therefore the step that produces it, its ledger rows, and its grant name | Opaque string, unique within `App.Artifacts`. **One identity per step:** the id is the only name a step answers to, and the description a flow gives a step when declaring it is not an identity at all. |
| `SignalId` | An orchestrator-observed durable boolean | **An independent namespace from `ArtifactId`.** Signals are never handler-writable. |
| `StepId` | One step within a flow | The step's result id: its `ArtifactId` when it produces an artifact, its `SignalId` when it completes on a signal. **Inside a flow the two are a single namespace** — registering a second step under an id already taken is refused whichever kind it is — so a `StepId` is unambiguous without carrying a discriminator. The two vocabularies stay independent everywhere else; it is the flow that merges them. Every `StepId` keys journal entries and ledger rows. |
| `RoleName` | One role a flow declares | **The flow's own vocabulary, carried verbatim** — the orchestrator stores it (journal entries, the awaited-role marker) and interprets none of it: which roles exist, which capabilities they require, and who may assume one are all derivations the SDK owns ([resolution.md](resolution.md) § Accounts, capabilities and roles). Same floor as `TagId` — non-empty, single-line, no surrounding whitespace — because it becomes a marker in the orchestrator's own store. |
...
**Availability is computed in the orchestrator, from knowledge that is partly the SDK's.** That is why `List` takes predicates at all: the ladder's third level — whether this item's type is in the binary's remit — and its fourth — whether this account may assume the role an item awaits — are the SDK's business, and the orchestrator cannot answer either alone. The predicates are how that knowledge crosses without the orchestrator importing the flow definition or role derivations.
...
Two things that look like identities and are not: a step's **description** (display only, deliberately not called a name, and never accepted where an id is expected), and a **block reason**, which is prose for a person and carries no reference — the references are `BlockedBy`. See "Dependencies".
...
| `ItemType` | What kind of item it is | The remit gate: a flow declares which types are its binary's work, and an item whose type is outside the remit is `outside-remit`. Consulted before the journal's first entry, never after ([flow-registration.md](flow-registration.md) § Item types). **Must be non-empty.** |
...
| `ItemScope` | How far up the listing ladder to report | Closed set of seven, named for the levels themselves: `all`, `open`, `processable`, `actionable`, `workable`, `free`, `auto`. |
| `Availability` | Where one item sits on that ladder | Closed set of seven: `auto`, `available`, `held`, `blocked`, `awaits`, `outside-remit`, `closed`. Each is exactly the boundary between two adjacent scopes. |
| `Priority` | Where the item sits in the order work is taken in, as whatever manages the work ranks it | Closed set of four, highest first: `critical`, `high`, `medium`, `low`. It ranks the **work**, and it is written by machines — a triage automation, a flow, whatever else maintains the work. **`medium` is what an item has when nothing has said otherwise**, so an item nobody assessed does not sort below one a machine deliberately marked `low`: silence is not an assessment. See "Priority and urgency". |
...
| `ParkKind` | Why a step stopped without completing | Closed set of ten: `blocked`, `question`, `treasurer-refused`, `step-did-not-complete`, `refused`, `infra-transient`, `remote-unreachable`, `write-contract`, `account-exhausted`, `condition-unmet`. The kind decides what clears the park: a grant clears **only** a treasurer-refused one, and only when the treasurer accepts the extension as clearing what it refused on. Every other kind is cleared by whatever caused it ending, or by an operator. **The kind also decides whether re-dispatch can clear it.** Four kinds can: `infra-transient` and `remote-unreachable` against a runner or remote that has come back, `step-did-not-complete` because a re-dispatch is exactly what does the job it left undone, and `account-exhausted` once the window has reset. The other six cannot — a `blocked`, `question`, or `treasurer-refused` park waits on a person or a grant, a `refused` or `write-contract` park is deterministic, answering identically every time, and a `condition-unmet` park waits on a fact outside the item that a dispatch neither tests nor changes — so re-dispatching one is a loop, not a retry. The classification is a function of the kind (`ParkKind.RedispatchMayClear`), enumerable so a crosswalk proves it handles each, and a parked result carries it outward as `redispatch_may_clear` ([cli.md](cli.md) § One-shot reports). **`account-exhausted` is the one kind that also knows *when*.** The agent account's allowance is spent and the substrate published the instant it returns ([environment.md](environment.md) § The agent account), so the park carries that instant and a parked result reports it as `clears_at`. It does not narrow the classification: re-dispatch may clear it, and the instant says when trying is worth anything. A driver that re-dispatches before it is looping against an answer it was given. |
| `ParkKind` (continued) | **`condition-unmet` is the kind that clears without anyone lifting it.** Its own member rather than a reading of `blocked`, on the reasoning `account-exhausted` was admitted by: nothing failed, nobody has a decision to make, and an operator sent to act would be looking at an item that resumes when a fact outside it becomes true. It differs from `account-exhausted` in what it waits on — a predicate over state anyone can read, not an instant a substrate published — so it carries a **clearing condition** rather than a `clears_at`, and a driver evaluates that condition before the next dispatch instead of counting down to it. It is the one kind that maps to a `BlockKind` of `waits-on-condition`, which had no park kind before it ([trunk-health.md](trunk-health.md) § Two conditions, two clocks). |
| `BudgetAxis` | Which resource the treasurer refused on | Names what a `treasurer-refused` park stopped on and what a `Grant` extends — time, cost, or a count the treasurer keeps. **The treasurer's vocabulary**, carried through verbatim; the orchestrator stores it and interprets none of it. |
| `Capability` | What an account can do on the repository | Closed set of three: `push`, `merge`, `approve`. Detected from the orchestrator, never declared — the input to role derivation ([resolution.md](resolution.md) § Accounts, capabilities and roles). |
| `PlacementAxis` | Which platform fact a placement restriction constrains | Closed set of two: `os`, `arch` ([environment.md](environment.md) § Placement). Values are the platform's own vocabulary, compared by exact equality. The arena's facts are measured where it runs, never declared; the item's restrictions are item-level facts, edited like any other. |
| `Disposition` | How a finalized item ended | Closed set of two: `resolved` and `rejected` ([resolution.md](resolution.md) § Finalizing). Carried on `Finalize` and reported on `Item`. |
| `ClaimOverride` | A safety check the operator chose to bypass | Closed set of five: `dirty-tree`, `already-held`, `unadmitted`, `stale-base`, `unmet-placement`. **flow's own vocabulary** — these are CLI flag names, not the orchestrator's, and an orchestrator receives them rather than defining them. |
...
| `TrunkState` | What is known about a trunk commit's soundness | Closed set of three: `green`, `red`, `unknown` ([trunk-health.md](trunk-health.md) § The three states). Scoped to a commit **and** a platform together — a commit `green` on one platform is `unknown` on another until measured there, and both answers are complete. **`unknown` is never read as `green`**: it reports that no mechanical measurement is on record, which is what a store holding none answers for every commit rather than refusing. |
| `BlockKind` | **Who must act, and on what**, for the item to become workable | Closed set of three. Each names an actor and a subject, which together are the only thing a caller does differently.<br>**`waits-on-items`** — actor: whoever can work the blockers. Subject: those items, named in `BlockedBy`. It clears when they finish and nobody touches this item at all.<br>**`waits-on-condition`** — actor: nobody. Subject: nothing addressable — a network away, a service down, a rate limit, an awaited signal not yet observed. It clears on its own and there is nothing to go work; the only response is to come back.<br>**`waits-on-person`** — actor: a person. Subject: this item — answer its question, grant its budget, re-enable it. It will not clear on its own.<br>The causes within each are open-ended; who acts and on what is not, which is why this is a field and the reason is prose. |
| `ClaimRefusalCode` | Why an orchestrator refused a claim or a release | **The orchestrator's own vocabulary**, carried through verbatim. flow deliberately defines no constants for it, because mirroring the orchestrator's protocol here would be a second enum nothing keeps in sync. |
...
| `Awaits` | What the item awaits — its stored marker: a `RoleName` with that role's account of record when one is bound, or the `SignalId` a pending signal wait is held on; empty when unstarted or finalized. An awaited signal reports the item `blocked` (waits-on-condition), never `awaits` — nobody's move is not somebody else's. |
| `Requires` | The item's placement restrictions: per `PlacementAxis`, the values that qualify ([environment.md](environment.md) § Placement). Empty means any arena. The other half of an `awaits` report. |
...
|---|---|
| `Ref` | The item's `ItemRef`. |
| `Type` | Its `ItemType`. **Must be non-empty** — it is what the remit gates on. |
| `Title`, `Body`, `URL` | The request as a person wrote it, and where to read it. |
| `Creator` | The `AccountId` that filed it. What a step routes on when the source's standing matters; its detected capabilities are asked for through `DetectCapabilities`, never taken from the item body's claims about itself. |
| `Flow` | The flow bound to it at the journal's first entry, when known. |
| `Status` | Its `ItemStatus`. |
| `Holder` | The arena holding it and the `AccountId` credited, or empty when unclaimed. |
| `Awaits` | The `RoleName` the item awaits and that role's account of record, as on `ItemInfo`. |
...
| `Ledger` | The treasurer's durable record: per-`StepId` dispatches, resumptions, cost, active and waiting durations, their totals, and what grants extended. |
...
| `ArtifactDef` | An `ArtifactId` **and** its `ArtifactType` — the pair is the schema, and both halves are validated. Plus a one-line description of what the artifact holds and when it is produced. An `ArtifactJSON` entry also carries its payload's declared shape — the id names one shape wherever it appears ([artifacts-and-signals.md](artifacts-and-signals.md)). |

### Writing payloads

| Type | Holds |
|---|---|
| `JournalEntry` | One completed step execution, as `AppendEntry` records it and `Item.Journal` returns it: the `StepId` and which execution of it this was; the captured result (an `ArtifactBody`, or the signal observation); the route elected — a successor `StepId`, or a `Disposition`; what the item now awaits (the successor's `RoleName`, or the `SignalId` when the successor is a signal wait — what the awaited marker is maintained from; the SDK computes it, because the step-to-role mapping is the flow's); the message to the successor; the standing note when one was set; the acting `AccountId` and `RoleName`; when it ran and what it spent. |
| `ArtifactBody` | The captured artifact value inside an entry. A union: it carries an `ArtifactType` and exactly one populated payload, and which one is legal is determined by that type. An empty body is legal — see `AppendEntry`. |
| `ItemDraft` | What `FileItem` creates from — what a person filing would supply: title, body, `ItemType`, tags — plus the **key**, stable within the source item's resolution, that idempotence matches on. |
| `Grant` | What `Grant` extends, in the treasurer's own axes: cost, additional time, or a count the treasurer keeps. Every axis is optional; a grant may raise one and leave the rest. |
| `ParkRequest` | Why a step stopped: its `ParkKind`, the `StepId` that parked, and a one-line reason with optional detail. When the kind is `treasurer-refused` it also carries the offending `BudgetAxis` **and the state of every axis** — the axes go flat together, so reporting only the tripping one costs an operator a grant per axis. When the kind is `account-exhausted` it carries the instant the allowance returns, and the identifier of the account it belongs to — a park naming neither is scoped to nothing and cannot be waited out ([environment.md](environment.md) § The agent account). When the kind is `condition-unmet` it carries what would clear it as structured values — the commit and platform whose verdict is missing, the base whose red verdict is being waited past, or the delivery that must be replaced — and never as prose a reader would have to parse the fact back out of ([trunk-health.md](trunk-health.md) § Three conditions, three clocks). |
| `AgentSession` | The resolution's handle on its agent conversation, and the boundary already honoured: the substrate's session identifier, or empty where the resolution holds none, plus the `StepId` whose `fresh` declaration this record has already acted on. The two travel together in one write so they cannot disagree — the boundary is what makes `fresh` a property of one **execution** rather than of each dispatch, so a fresh step that parks and is re-dispatched resumes the session it opened instead of opening a second ([resolution.md](resolution.md) § The agent session). |
| `SessionReason` | Why a new agent session was asked for, closed at three: `declared` — the route asked for it, the resolution's first or a step declaring `fresh`; `handle-gone` — nobody's decision, the substrate declined the handle or the orchestrator keeps none; `refused` — machinery asked to discard a live conversation the route does not account for, and **nothing was opened**. The set is what tells the two causes apart: one is a defect in the flow, the other a limit of what it runs on, and they have different fixes ([resolution.md](resolution.md) § The treasurer). |
| `SessionCounts` | The ledger's fourth column: how many sessions the resolution opened under each reason, and how many requests it refused. `Opened` is the declared and handle-gone counts summed; a refusal opened nothing, so it is not in it. **Tallies rather than a log of every request** — an orchestrator with nowhere to keep a handle opens a session per dispatch, so a record of each would grow the durable state without bound while answering nothing the buckets do not. |
...
### `TrunkHealth` — what is known about a trunk commit

Returned by `TrunkHealthAt` and `GreenTrunkAhead`, recorded by `RecordTrunkHealth`. Carries the `CommitSha` it is about, the platform it was reached on — the `PlacementAxis` values, never a vocabulary of its own — its `TrunkState`, the tree id the measurement was taken over, and when it was taken. A `TrunkState` of `unknown` carries the commit and the platform and nothing else, there being no measurement to describe. The model is [trunk-health.md](trunk-health.md); this is its shape on the boundary.
...
| `ArenaRoot()` → `string` | The **absolute** path of the arena's checkout: the directory every process the SDK spawns for this arena runs in — gates, commands and the agent prompt alike. It is the same value the orchestrator's own git operations use, read off the one place it is held, so the tree the agent edits and the tree the commit is taken in cannot diverge. Fixed when the orchestrator is constructed, and **never the process working directory** — see `Claim`, where the same rule is stated for the arena identity this path is. Empty only where the orchestrator has no local checkout at all; like `SupportedGates`, an empty answer is an honest declaration, not a refusal. |
...
| `List(ctx, ItemScope, BinaryName, func(ItemType) bool, func(RoleName) bool)` → `([]ItemInfo, error)` | Items at the given `ItemScope`, with per-item availability, tags, holder and blockers. The `BinaryName` names the label that marks an item opted in, which is what separates `auto` from `available`. The predicates are **not values at all**: each is the SDK lending the orchestrator its own knowledge — the flow's remit, without which the orchestrator could not tell `outside-remit` from `processable`, and whether this account may assume the role an item awaits, without which it could not tell `awaits` from `actionable`-and-above. The placement half of that boundary the orchestrator derives itself — its own arena's facts against the item's restrictions, both of which it holds. **At scope `auto` it must return items in selection order**; at every other scope the order is not a contract. See "Priority and urgency". Feeds `list`. The auto-select path **must never** call it. |
| `Get(ctx, ItemRef, BinaryName, func(ItemType) bool, func(RoleName) bool)` → `(*ItemInfo, error)` | One of exactly what `List` returns, addressed by ref instead of enumerated. **It must answer identically to `List` for the same item at the same moment** — one derivation serving both, never two. An item that reads `blocked` in `list` and `available` in `status` is a contradiction an operator cannot resolve, and nothing in the item caused it. Feeds `status`'s availability line, and is what makes an `ItemRef` sufficient — without it, listing metadata is reachable only by enumerating everything and searching. |
| `ListAutoSelectable(ctx, []TagId, func(RoleName) bool)` → `([]ItemRef, error)` | The items an unattended `resolve` may start on, carrying every `TagId` given. An empty list means no filter. Feeds the auto-select path. **Must not** return a `blocked` item, **must not** return a `deferred` one, **must not** return one **another arena holds** — including one holding it under this same account, since the lease binds `item ↔ arena` and one operator running several worktrees writes one `AccountId` from all of them — and **must not** return one whose awaited role fails the predicate or whose placement restrictions this arena does not meet — somebody else's move, and somebody else's machine, are not this runner's work. Each of these is **eligibility, not a sort key**: an item another arena is running is absent from the set, never merely ranked last, because a caller handed it can only try to claim it. **Must** return what it does return in selection order. Filtering and ordering both belong here because tags, priority, urgency and the awaited-role marker live in the orchestrator and `ItemRef` carries none of them — a caller has nothing to filter or sort on. See "Priority and urgency". An orchestrator with no tag vocabulary returns nothing when tags are given, which is an honest answer. See "Dependencies" below. |

### Claiming

| Method | Contract |
|---|---|
| `Claim(ctx, ItemRef, []ClaimOverride)` → `(Claim, error)` | Acquires an exclusive lease binding `item ↔ arena`, one-to-one in both directions (at most one item per arena, at most one arena per item). **Neither the arena nor the account is a parameter.** Both are ambient in the sense that neither is passed in: the arena is the checkout the binary belongs to, fixed once when the orchestrator is constructed, and the account is whoever that arena's credentials act as. **Ambient never means the process working directory.** A location taken from where a process happened to be started is not stable across restarts — the same checkout invoked from two directories is two arenas, two checkouts invoked from a common parent are one — so the arena is derived from something that does not move, such as the binary's own location, or configured as an absolute path. An arena must commit, push and merge, so it always has exactly one account, and the orchestrator can read it — a caller-supplied one could only agree or be wrong. `[]ClaimOverride` names safety checks the operator chose to bypass. **The binding holds from the claim until the item is finalized or the lease is explicitly released** ([resolution.md](resolution.md) § Claiming): it survives a step parking, a run ending, the process being killed and the machine restarting, because none of those resolved the item. So an orchestrator **must refuse a claim from an arena that already holds a different item**, with a code of its own, distinct from `already-held` and **not reachable by any override** — see "The binding is durable" below. Re-claiming the item this arena already holds is unaffected: it stays idempotent and changes nothing. |
| `Release(ctx, ItemRef, []ClaimOverride)` → `error` | Relinquishes the lease — **the exception to the binding's lifetime, not a step in it.** It is the deliberate breaking of a binding that would otherwise end at resolution, so it must leave the arena genuinely free for the next item. An orchestrator with a local checkout **must therefore refuse a release while the worktree is dirty or while HEAD is off the base branch**, typed with the same codes `Claim` uses for those two conditions. The ordinary way past that refusal is git, deliberately: commit the work to the item's branch or discard it, return to the base, release. That is the moment somebody decides what happens to the work, instead of it becoming nobody's. **The overrides come from the same closed set `Claim` takes**, and no member is added for this direction: `dirty-tree` and `stale-base` bypass the two preconditions, and `already-held` is what permits releasing a record this arena does not hold. What a bypass costs is exactly the orphaned state the refusal prevents — uncommitted work no item holds and no branch carries, in a tree an unforced claim then refuses — so it is the operator's emergency and never something a run reaches for: an arena being decommissioned, a lease record nothing can read, a claim whose arena cannot be reached. **A ref whose record on the item names another arena or another account is refused without `already-held`**, because that arena's tree cannot be read from here and the two preconditions therefore cannot be evaluated against the arena the release would free — passing them by default would be answering a question nobody asked. What decides it is the record **on the item**, not whether this arena's lease happens to name the ref: a record naming *this* arena with no lease left to match it is this arena's own to take apart, and is exactly the state an arena reimaged, or a worktree deleted with the item still labelled, leaves behind. **An item carrying no claim record at all is refused as `not-claimed`, with no override** — the account half is what says a lease was taken ([github-schema.md](github-schema.md) § Labels), so there is nothing a release could take apart and nothing an override could reach; reporting a release instead would let a mistyped id edit an item the flow never claimed. That refusal is conditioned on this arena's lease not naming the ref either, so an arena finishing a teardown that stopped between the last removal and the clearing is unaffected. **A ZERO `ItemRef` names no item**: the arena's own record alone is taken apart and nothing on any item is touched or read, which is how an arena whose lease record cannot be *parsed* is freed — a record nothing can read names no item to release. **The arena's own record is cleared only when its lease can be READ and says the arena holds this item or holds nothing**, because that record is arena-wide — the lease, the drafts and the session go together. Clearing it while releasing somebody else's claim would destroy the state of whatever this arena is part-way through, which is the waste the preconditions exist to prevent ([resolution.md](resolution.md) § Nothing is bought twice). **A lease that cannot be read is not a lease naming nothing**: the drafts and the session are separate records that a torn write to the lease leaves intact, so an arena that cannot say what it holds may be holding a whole resolution's reasoning — a release addressed at some *other* item leaves that lease where it is, and the zero-ref release is the deliberate way to clear it. **An arena that is genuinely gone** — a machine that will not come back — is recovered from *another* arena with the `already-held` override, on `Claim` when the item is to be worked and on `Release` when the record is only to be dropped; both are the emergency path, and both are audited. A **displaced** arena, one whose item another arena took over, is held to both preconditions like any other — displacement is no exemption, and such an arena is the likeliest to be dirty and off the base, having been mid-work when its item was taken. What the displacement changes is only **what an accepted release takes apart**: the arena's own record alone, never the taker's claim on the item. Displacement is *this arena's lease naming the item while the item names another arena*, both halves: an arena that does not hold the item is not displaced by it, and a release it addresses at that item takes the record apart rather than its own lease. |
...
| `DetectCapabilities(ctx, AccountId)` → `([]Capability, error)` | What the named account can do on the repository, asked of the orchestrator. For the ambient account it feeds role derivation before a claim; for an item's creator it feeds routing on the source's standing. **The empty `AccountId` names the account the orchestrator acts as** — the same ambient account `Claim` credits, fixed by where the call runs. It has to be spellable, because this is the one question asked *before* a claim exists, and `LookupActiveClaim` — the only other route to that account — has nothing to answer with yet; a caller-supplied ambient account could only agree or be wrong, for the reason `Claim` takes none. **Detected, never declared**: the answer is the orchestrator's, and an orchestrator that cannot ask reports that rather than guessing. An account with nothing on the repository is an empty set, not an error — "may do nothing here" is an answer role derivation acts on, where "could not ask" is not. |

#### The binding is durable, because the state lives in the arena

Once an item is claimed to an arena, **that arena is the item's arena for the item's whole life**, and it is occupied for that whole time. Not until the run ends, not until a step parks, not until the process is killed or the machine restarts — a park does not break the binding, and a parked item is still this arena's item, with its work still in this arena's worktree, waiting to continue.

The reason it is durable rather than merely tidy is that **an item's resolution state is not all on the orchestrator.** A large part of it is local to the arena: the active claim, the worktree, the item's branch, the changes a step has made and not yet committed, the build outputs a gate will measure. None of that is addressable from anywhere else, and none of it survives being displaced.

So claiming a second item into an occupied arena does not merely overwrite a bookkeeping record. It either **mixes the two items' states** — item B's steps running against a worktree carrying item A's uncommitted work — or **loses A's state outright**, with the lease record replaced and A's changes stranded in a tree that now answers to B. Neither is recoverable by re-reading anything, because the lost part never existed anywhere but there.

The reverse direction has the same root. An item may be claimed in **at most one arena**, so that two arenas cannot resolve the same item at once, each accumulating local state the other cannot see, and each producing a branch and a proposal for the same work.

That is what the one-to-one binding is load-bearing for: it is the only thing keeping resolution state and the place it lives in agreement. It is also why the two refusals above are the shape they are. An occupied arena is not a contest over the item — there is nobody to take it from, and the item named in the refusal is this arena's own — so the override that exists for taking an item from another party has nothing to do here, and applying it would orphan the first item rather than resolve a dispute. And a release is not a claim in reverse: a claim that starts on a dirty tree can at least be attributed to the item it took, where a release leaves **no item at all** to attribute the tree's contents to.

**An orchestrator whose lease record is not what scopes the arena must still answer both questions.** "What does this arena hold?" and "who holds this item?" are separate reads with separate stores — a worktree-local file answers only the arena that wrote it, so the item must carry the holding arena's identity too, or two arenas under one account each see their own answer and both claim.
...
| `Reset(ctx, ItemRef)` → `error` | Clears the flow's whole record on the item — journal, ledger, park, questions' outstanding marker, drafts, **and the agent session** — so the next resolution starts from an empty journal. The session goes with the record it belonged to: a conversation kept past the journal it accompanied has nothing left to continue, and the next dispatch would resume it against a wiped record. Operator-initiated only; the SDK never calls it automatically. An orchestrator that cannot clear its record refuses; the refusal is typed, so a caller can tell *not supported* from a transient failure. |
...
| `AddRequirement(PlacementAxis, value)` | Records a placement restriction: only arenas whose axis fact is among the recorded values qualify. Adding one already recorded changes nothing. |
| `RemoveRequirement(PlacementAxis, value)` | Retracts one. Removing one not recorded changes nothing. |
...
- **Tags** are added and removed rather than assigned as a set, because an item's tags have more than one author — the operator's classification, and the markers an orchestrator maintains as consequences of contract operations. Assigning the set entire would silently delete whichever half the caller did not know about. **An orchestrator must refuse to remove a marker it maintains itself**: the owner, binary and blocked markers follow from `Claim`, the flow's own writes and a blocker declaration, and a caller able to delete one directly could make an item report a state no operation put it in.
...
- **Manual** stops anything dispatching the item underneath the person now driving it, and setting it resolves any unresolved park — taking hand control **is** the decision to continue, and a park left standing would advertise a condition the person who took over is the one clearing. No command asserts it: `run-step` is a primitive an external scheduler drives in a loop, and marking the item there would flag it for a person who is not there ([cli.md](cli.md) § Advancing one step). Clearing it returns the item to automatic dispatch; an item that could be taken over and never handed back would be stranded by the act of helping it. `Load` **must** report the current value.
...
| Method | Contract |
|---|---|
| `FileItem(ctx, ItemRef, ItemDraft)` → `(ItemRef, error)` | Creates one item in the orchestrator's own store as a step's recorded deliverable — the **only** method on this surface that brings an item into being. It exists so outward creation crosses the same guarded seam as every outward write ([disclosure.md](disclosure.md)) and carries provenance: the created item is stamped with the source `ItemRef` and the draft's key. **Idempotent by (source, key)**: called again for a pair already filed, it returns the existing item's ref and changes nothing — which is what makes a filing step resumable without duplicating ([archive/issue-flow.md](archive/issue-flow.md) § File the items). The created item is an ordinary item; everything in this contract applies to it. |

### Journal

| Method | Contract |
|---|---|
| `AppendEntry(ctx, ItemRef, JournalEntry)` → `error` | Appends one completed step execution: the captured result and the elected route, in one write — an entry is never half-recorded, and an existing entry is never rewritten. There is no orchestrator method for writing signals — signals are written by orchestrator-internal side effects or the `Load` poll path. An empty artifact body is a legal entry where the orchestrator can verify the content it stands for exists (a capture stored elsewhere, a side effect it can observe); it verifies, and fails naming what is missing. |

The journal is the durable route ([resolution.md](resolution.md) § The journal): `Load` **must** return exactly what was appended, in order, because the pending step is derived from the last entry and a journal read short or out of order re-routes the item.

### Ledger

The treasurer's durable record. Every row is keyed by `StepId` — the result a step produces is that step's identity, the ledger rows hang off it, and it is the only name `grant` accepts. The rows are transactional with the writes they account.

| Method | Contract |
|---|---|
| `RecordDispatch(ctx, ItemRef, StepId)` → `error` | Counts one dispatch of the pending step. |
| `RecordResumption(ctx, ItemRef, StepId)` → `error` | Counts one resume of a parked step — the unpark count the treasurer reads. |
| `AddCost(ctx, ItemRef, StepId, float64)` → `error` | Adds cost to the step's running total. |
| `AddDuration(ctx, ItemRef, StepId, time.Duration)` → `error` | Adds **active** time — time spent doing work — to the step's running total. Reported alongside cost when present. Time blocked on a declared exclusion goes to `AddWaiting`, never here. |
| `AddWaiting(ctx, ItemRef, StepId, time.Duration)` → `error` | Adds time spent blocked on a declared exclusion — recorded apart from active time, as evidence about contention rather than about the work ([resolution.md](resolution.md) § The treasurer). Reported by the party that held the wait, the only party that knows it was one: an orchestrator whose `Push` waited its turn reports that wait here. |
| `RecordSession(ctx, ItemRef, SessionReason)` → `error` | Files one request at the treasurer's third chokepoint under its reason: a session the route declared, a session opened because the handle was gone, or a refused request that opened none ([resolution.md](resolution.md) § The treasurer). **It takes no `StepId`**, and that is the difference from every other ledger write — see below. The reason-to-count mapping is `SessionCounts.Record` and nowhere else; an orchestrator that switched on the reason itself would be a second answer to which figure a reason moves. |
| `Grant(ctx, ItemRef, StepId, Grant)` → `error` | Records an operator's extension. **Must clear** a `treasurer-refused` park when the treasurer accepts the extension as clearing what it refused on. Parks of any other kind, and extensions the treasurer finds insufficient, **must** be left in place. The decision is the treasurer's and the SDK provides it — an orchestrator must not re-derive when a grant is sufficient. |

**The session count is the item's, not a row's.** Every other ledger write names a step, because what it records is that step's. A session belongs to the **resolution** — it outlives every step on the route ([resolution.md](resolution.md) § The agent session) — so there is no row to hang the count off, and `RecordSession` takes no step for the same reason `SaveAgentSession` is keyed by the item alone. A count filed against whichever step happened to be running when a conversation was opened would report the step's spending rather than the resolution's.

The orchestrator stores the ledger and decides none of it: admission, allowances and refusals are the treasurer's ([resolution.md](resolution.md) § The treasurer), computed over the journal and ledger the orchestrator returns. Where the treasurer runs is the drive model's business; what it reads and writes crosses this boundary.
...
| `SaveDraft(ctx, ItemRef, StepId, string)` → `error` | Stores a step's draft when it stopped without completing, so the next dispatch does not start from nothing. **Never published** — the draft is the step's own scratch state, and for a refused write the text to store *is* the text a guard refused. See [resolution.md](resolution.md) § Drafts. |
| `LoadDraft(ctx, ItemRef, StepId)` → `(string, error)` | Returns what was stored, or empty. |
| `ClearDraft(ctx, ItemRef, StepId)` → `error` | Discards it. Clearing what is not there is not an error. |
| `SaveAgentSession(ctx, ItemRef, AgentSession)` → `error` | Stores the resolution's handle on its agent conversation, so the next dispatch continues it instead of buying it again. **Keyed by the item alone** — see below. **Never published**, for the reason the draft is not: the handle names a conversation holding the resolution's whole reasoning. See [resolution.md](resolution.md) § The agent session. |
| `LoadAgentSession(ctx, ItemRef)` → `(AgentSession, error)` | Returns what was stored, or the zero value. Absence is the zero value and not an error; a record naming a different item is not this resolution's and reads as absence too. |
| `ClearAgentSession(ctx, ItemRef)` → `error` | Discards it. Clearing what is not there is not an error. |
...
**The session is keyed by the item alone, and the draft is keyed by item and step.** That is the point of the session store: a draft belongs to the step that stopped mid-way, while the conversation belongs to the **resolution** — its steps continue it, whether the next runs in the same process or in a separate invocation on another day. Keying the session like a draft would end it at the first step boundary, which is the failure the store exists to close. Their lifetimes differ for the same reason: a draft is discarded when its step completes, and the session outlives every step on the route. Both live with the claim and are cleared when it is released or the item finalized ([resolution.md](resolution.md) § Drafts) — reasoning left behind after the work is over is a disclosure sitting around for no benefit.

**An orchestrator with nowhere to keep a handle answers a permanent typed refusal** rather than pretending to store one. It is then expensive rather than incorrect: every dispatch opens a session, every step starts from the prompt it was given, and nothing is wrong — the handle is offered and never depended on ([resolution.md](resolution.md) § The agent session), so the mechanism is absent rather than broken.
...
| Method | Contract |
|---|---|
| `Finalize(ctx, ItemRef, Disposition)` → `error` | Marks the item's flow run complete — with the disposition the finalizing election carried — and releases its claim. **Must refuse an item whose `ItemStatus` is not `terminal`.** Finalizing does not *make* an item terminal; there is no method here that closes one, and an item reaches terminal by the orchestrator's own means. So `Finalize` records that the flow is finished with an item already finished, and refusing an open one is what keeps the two facts from drifting: a finalized item still open claims the work is over while the orchestrator says it is not. **This is the only way completion is ever recorded** — nothing infers it. A finished item and an item with no currently eligible step are otherwise identical, so an orchestrator that could not finalize could end nothing: selection would keep offering the item and `status` could never say it was done. Its read is required with it — `Load` **must** report `Item.Finalized` truthfully, because a write nothing can observe is not a record. |

### Trunk health

| Method | Contract |
|---|---|
| `TrunkHealthAt(ctx, CommitSha)` → `(TrunkHealth, error)` | What is known about that commit **on this arena's platform**. The platform is ambient for the reason the arena is on `Claim` — it is measured where the call runs, and a caller-supplied one could only agree or be wrong. A caller able to name *another* platform would be handed a verdict its own gate never has to agree with, which is the misreading [trunk-health.md](trunk-health.md) § A verdict names the platform it was reached on exists to prevent. **A commit with nothing on record answers `unknown`, not an error**, and so does every commit on an orchestrator with no store: absence is an answer a step acts on, where a refusal is not. |
| `GreenTrunkAhead(ctx, CommitSha)` → `(*TrunkHealth, error)` | The nearest trunk commit ahead of the given one that is `green` on this arena's platform, or nil when there is none. It is what clears a step blocked on a red base, and what the rebase that follows targets ([trunk-health.md](trunk-health.md) § Three conditions, three clocks). **A commit that is ahead but unmeasured is not an answer**: handing one back sends an arena to rebase onto a base it knows nothing about, which is the same stop one commit later. Nil is an ordinary answer and never an error. |
| `RecordTrunkHealth(ctx, TrunkHealth)` → `error` | Records one verdict against one commit on one platform. **Must refuse a verdict whose tree id is not the commit's tree** — the measurement is about content, and a commit squashed or rebuilt since is not the content measured ([trunk-health.md](trunk-health.md) § The verdict is keyed to what landed). **Must refuse a verdict contradicting one already recorded** for that commit and platform, typed distinctly from the first refusal: a stale measurement and two live ones disagreeing are different facts with different fixes, and silently taking the later one destroys the evidence that they disagreed. Re-recording an identical verdict changes nothing and is not an error. |
...
| `Push(ctx)` → `error` | Publishes the branch. **May wait** — the orchestrator serializes landing across everything sharing the mainline. Waiting is not failing, and the wait is reported to the ledger as waiting, never as work. |
...
| `CutPoint(ctx, base BranchName)` → `(CommitSha, error)` | The commit the current branch left `base` at — the merge base of `HEAD` and `base`, where `base` is **the item's base branch**, passed in as on `Branch`, and not necessarily the repository's default. **On a branch carrying nothing it is `HEAD`**, including when the base has advanced since the cut, which is why the answer is a merge base and not the base branch's tip; on a branch that already carries commits it is behind `HEAD`. That is the distinction `RevParse` cannot make — there `HEAD` is the base *plus* the work — and it is what tells a branch carrying work from an empty one. An implementation that cannot compute one — a base that will not resolve, a history sharing no commit with it — **returns an error rather than `HEAD`**: a caller handed `HEAD` would conclude the branch is empty. |
| `Drift(ctx)` → `(Drift, error)` | How far the world has moved under the branch: commits the branch carries past its recorded cut point (`Ahead`), commits the mainline has taken since that cut point (`Behind`), and when the reading was taken. **A measurement, already stale when returned** — evidence for judgment, never a safety check; see below. |
| `Run(ctx, CommandName)` → `(CommandRun, error)` | Runs the named command — one of the three `SupportedCommands()` declares. **May wait** where the project declared the command host-scoped ([gates-and-commands.md](gates-and-commands.md) § Two scopes); waiting is not failing, and the wait is reported to the ledger as waiting, never as work. **May modify the worktree or the arena environment**, which is what makes it a command, and why **no landing decision rests on what it reports**. Like `RunGate`, it returns a run rather than a bare error: the `Outcome` separates *ran and reported* from *could not start*, *timed out* or *died*, so a caller can tell a failing check from a command that never executed. A non-nil error means no command was run and no outcome exists. |
| `RunGate(ctx, GateName)` → `(GateRun, error)` | Runs the named `GateName` and returns a `GateRun`, whose `Outcome` distinguishes a real measurement from a gate that timed out, could not start, died, or broke its contract and reports what the runner observed. The SDK is the runner. A non-nil error means no gate was run and no outcome exists. A gate **modifies nothing it measures** — tree or environment. **May wait** where the project declared the gate host-scoped ([gates-and-commands.md](gates-and-commands.md) § Two scopes); waiting is not failing, and the wait is reported to the ledger as waiting, never as work. `RunGate(ctx, "integration")` is the landing gate — see below. See [gates-and-commands.md](gates-and-commands.md). |
...
**Act-safety is race-sensitive, and no distance reading may carry it.** *May this push proceed, is this measurement still about what will land* — those are true at the reading and false at the act, and each has its own answer already: whether the branch carries work is `HEAD` against the recorded cut point, which does not move ([archive/issue-flow.md](archive/issue-flow.md) § Open branch); whether the change still lands is the gate on the merge result `PrepareMergeResult` sets up; whether a push is safe is the push itself, the one act atomic with respect to other landings; a claim over a stale base is refused at claim time (`stale-base`). What dissolves under those answers is the election between two *mechanical* spellings of landing — push now versus rebase-then-push: bringing a branch up to the mainline is idempotent, a no-op at head, so a mechanical lander performs it unconditionally inside the bounded landing loop, and the serialized round keeps its measurement from being invalidated ([gates-and-commands.md](gates-and-commands.md) § Two scopes) — checking first would shrink a race window the lock closes.

**Whether there is judgment work to do before landing is a different election, and it is `Drift`'s legitimate one.** The pattern is ordinary practice: at head, landing is mechanical — verify and push. Behind, a rebase is not just a command: its result deserves review — a clean application can still be a wrong program, and a conflicted one is exactly the judgment [resolution.md](resolution.md) § Duplicate-fix conflicts legislates — so the route goes through a step that rebases and **reviews the rebase** before returning to the mechanical land. Electing between those routes on `Drift` is judgment on evidence, like every judgment the treasurer makes on counts, and the race is absorbed as a route rather than fought: a reading that went stale costs one hop — the mechanical lander, finding under the lock that the world moved after all, elects back to the rebase-review step with that finding as its message, an ordinary journaled election. A stale reading never lands anything wrong, because the push arbitrates; never burns a measurement, because the lock spans the round; it costs a dispatch, and the journal says why.

The ordinary producer of a large reading is not traffic mid-round — the lock excludes that — but **resumption**: the landing lock never survives a stop ([gates-and-commands.md](gates-and-commands.md) § Two scopes), so an arena whose work parked, crashed, or sat through an orchestrator's downtime comes back arbitrarily far behind what others landed meanwhile, and re-enters through this election. That is not an edge case; it is what stopping mid-landing costs, paid by the stopped work rather than by the queue.

What a step must never do with the reading is treat it as establishing that an act is safe — the split is the command rule one level down: evidence anyone may read, and decisions about acting that may not rest on it.
...
| `Merge(ctx, RequestUrl)` → `error` | Merges the pull request named by that URL. **It merges; it does not queue a merge** — on return the request is merged and `FindPR` can read the merge commit it produced, which is the only place a caller recording what landed can read it from. An implementation that returns having only asked for the merge answers for a landing that has not happened, and because a merge completes a signal rather than an artifact, nothing waits on it: the step is dispatched again. **It is the landing act here, so it ends the landing round and gives the project-scope exclusion back** — on a refused merge as well as a successful one, because a round that landed nothing is over too. It **may wait** where nothing opened the round before it; waiting is not failing, and the wait is reported to the ledger as waiting, never as work. May trigger orchestrator signals (e.g. `pr-merged`). |
| `FindPR(ctx)` → `(PRInfo, error)` | The pull request for the current claim branch. |
| `PrepareMergeResult(ctx, BranchName)` → `error` | Set the tree to reflect the merge result, so a gate measures what will actually land rather than the branch in isolation. **It opens the landing round, so it is where the project-scope exclusion is taken** ([gates-and-commands.md](gates-and-commands.md) § Two scopes): the serialized section is the round, and a lock over the landing act alone leaves the measurement this sets up open to invalidation. It **may wait**, on the terms `RunGate` states. A preparation that FAILED ends the round — it measured nothing and will land nothing. |
| `RevertMergePrep(ctx)` → `error` | Undo that preparation. **It does not end the landing round**: undoing the local merge is what lets the branch be pushed as the branch, so it falls between the measurement and the land, and releasing there would reopen exactly the window the exclusion closes. |
...
- **An empty artifact body** (when the orchestrator stores the bytes). An orchestrator that carries content elsewhere verifies the capture it stands for exists and fails naming what is missing.
- **An append for an unclaimed item**, or one claimed by another arena. The item is the subject, so the item is the address; holding the lease is a precondition the orchestrator checks, not a value the caller supplies.
...
- **A release addressing a claim recorded for another arena or another account.** Unless the operator passes the `already-held` override: the holding arena's tree cannot be read from here, so the two preconditions above cannot be evaluated against the arena the release would free. Typed with the same code `Claim` uses for the same record, because it is the same comparison read at the other end of the lease. A record naming *this* arena is not this — it is this arena's own, whatever the lease says.
- **A release addressing an item that carries no claim record**, where this arena's lease does not name it either. **No override**: nothing was taken, so there is nothing to drop and nothing a flag could reach. Typed distinctly from `already-held`, because the actions differ — *decide whether to take that arena's record apart* against *you have the wrong item*.
...
`Availability` cannot answer it. The ladder is ordered and each item gets one state at the lowest boundary it fails, so `closed` and `outside-remit` both sit below `blocked`: an item outside this binary's remit reports `outside-remit`, and a closed one reports `closed`, each of them blocked or not. Reading `Availability == blocked` therefore answers a narrower question — *is this blocked and otherwise workable by me* — which is the right question for selection and the wrong one for an operator asking why an item is stuck.
...
Auto-selection takes items in an order, and two parties have a say in what that order is. `Priority` is the rank whatever manages the work gives it; `Urgency` is what a person wants done about it now. They are two fields because they are two parties.
...
**Deferral is not a rung on the availability ladder.** A deferred item is open, in remit, this operator's move here, unblocked, free and opted in — every boundary passed. What is true of it is that auto-selection will not take it, and that is exactly the boundary `available` already marks; `Urgency` is what distinguishes *nobody opted this in* from *an operator deferred it*. Giving deferral its own state would put two states on one boundary, which is the same mistake as splitting `blocked` by cause, and the ladder is closed against both for the same reason. See [cli.md](cli.md).
...
**`FileItem` aside, no method here brings an item into being.** Items originate outside this contract — filed by a person, or by an agent at a terminal — through the orchestrator's own interface. This surface reads them, edits them, leases them and records work against them, and every one of those acts presupposes an item that already exists. `FileItem` is not an exception to the reason behind the rule but its application: where creating items *is* a flow's deliverable, the creation has to happen where the guard and the provenance stamp are — never on a side path shelled around them.
...
- [trunk-health.md](trunk-health.md) — what a trunk verdict means and what a step does with one.
