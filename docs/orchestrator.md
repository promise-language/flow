...
| `ListAutoSelectable(ctx, []TagId, func(RoleName) bool)` → `([]ItemRef, error)` | The items an unattended `resolve` may start on, carrying every `TagId` given. An empty list means no filter. Feeds the auto-select path. **Must not** return a `blocked` item, **must not** return a `deferred` one, and **must not** return one whose awaited role fails the predicate or whose placement restrictions this arena does not meet — somebody else's move, and somebody else's machine, are not this runner's work. **Must** return what it does return in selection order. Filtering and ordering both belong here because tags, priority, urgency and the awaited-role marker live in the orchestrator and `ItemRef` carries none of them — a caller has nothing to filter or sort on. See "Priority and urgency". An orchestrator with no tag vocabulary returns nothing when tags are given, which is an honest answer. See "Dependencies" below. |
