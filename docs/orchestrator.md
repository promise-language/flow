...
- **Manual** stops anything dispatching the item underneath the person now driving it, and setting it resolves any unresolved park — taking hand control of a parked item is what resuming it means. **It is set by a deliberate act and never as a side effect of advancing the item.** A command that runs one step runs a step; inferring a takeover from which command was typed flags every unattended caller's item as hand-driven and clears the very parks that were stopping it, on behalf of a person who is not there. Clearing it returns the item to automatic dispatch; an item that could be taken over and never handed back would be stranded by the act of helping it. `Load` **must** report the current value.
