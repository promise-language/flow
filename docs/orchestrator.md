...
| `Release(ctx, ItemRef)` → `error` | Relinquishes the lease — **the exception to the binding's lifetime, not a step in it.** It is the deliberate breaking of a binding that would otherwise end at resolution, so it must leave the arena genuinely free for the next item. An orchestrator with a local checkout **must therefore refuse a release while the worktree is dirty or while HEAD is off the base branch**, typed with the same codes `Claim` uses for those two conditions, and with **no override**: a flag that dropped the lease and left the tree as it was would produce exactly the orphaned state the refusal prevents — uncommitted work no item holds and no branch carries. Nor would it leave the arena free, because the arena's state is the tree the release left behind: an unforced claim refuses that tree, and the claim that does succeed on it is one carrying the dirty-tree override, which starts the next item on the last one's leftovers. The way past a refused release is git, deliberately: commit the work to the item's branch or discard it, return to the base, release. That is the moment somebody decides what happens to the work, instead of it becoming nobody's. **An arena that is genuinely gone** — a machine that will not come back — is recovered from *another* arena with the `already-held` override on `Claim`; that is the emergency path, and it is audited. A **displaced** arena, one whose item another arena took over, is held to both preconditions like any other — displacement is no exemption, and such an arena is the likeliest to be dirty and off the base, having been mid-work when its item was taken. What the displacement changes is only **what an accepted release takes apart**: the arena's own record alone, never the taker's claim on the item. |
