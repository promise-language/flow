...
**There is no override.** A flag that dropped the claim and left the tree as it was would produce exactly the orphaned state the refusal exists to prevent: uncommitted work that no item holds and no branch carries. Nor would it leave the arena usable afterwards. An unforced `claim` then refuses that same tree — so the claim that would actually follow a forced release is `claim --force`, which overrides the dirty-tree check along with the others, and starts the second item on the first one's leftovers. Between those two, a forced release either strands the work and blocks the arena, or strands the work and carries it into the next item. The way past a refused release is git, deliberately — commit the work to the item's branch, or discard it; return to the base branch; release. That is the moment somebody decides what happens to the work, instead of it becoming nobody's. A parked item legitimately has uncommitted work in the worktree, so releasing one means dealing with that work first. **That is the intended outcome, not an obstacle.**
