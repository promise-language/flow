...
**`release <item-id>` drops the claim the named item carries.** The id is resolved the way `claim <id>` and `status <id>` resolve theirs (§ Addressing an item). When the item names this arena it *is* the bare form, checks and all. When it names another, that arena's tree, drafts and session cannot be read from here at all — so the two refusals cannot be evaluated against the arena the release would actually free, and it is refused rather than passed by default; `--force` is what carries it, and is the operator saying they know what is there. This is the route to a record that outlived its arena: one reimaged, one whose worktree was deleted with the item still labelled, or a claim rolled back after the record posted.
